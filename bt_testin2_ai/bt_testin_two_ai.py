...
ser_rfid = serial.Serial('COM8', 9600)#----------------------------------rfid的鏈接
...
#/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  （以下）   儲存資料部分
        if float_array_2d_hight == array_hight:
            ...
            horn_num = 0
            for i in range(array_hight):###################計算一列有多少喇叭
                horn_num = horn_num + int(float_array_2d[i, 1])
            print(horn_num)
            sample_long = copy.deepcopy(float_array_2d.reshape(1, 10, 4))
            sample_long[:, :, 1] = 0 #change horn
            ...
            if preds_long + 1 == 8:
                ...
                pred_text_long = 'no snaking'
            # print(pred_text_long)
        ...
        if float_array_2d_short_hight == array_hight_short:
            ...
            sample_short = copy.deepcopy(float_array_2d_short.reshape(1, 5, 4))
            sample_short[:,:, 1] = 0#change horn
            ...
            if preds_short + 1 == 1:
                ...
                if keys[pygame.K_w]:
                    ...
                    pred_text_short = 'backward ' + pred_text_short   
            if (not(('normal' in pred_text_short)or('left signal left' in pred_text_short)or('right signal right' in pred_text_short)or('right_signal' in pred_text_short)or('left_signal' in pred_text_short))):
                if (danger_flag == 0) or(not(prev_pred_text_short == pred_text_short)):#1.5秒後 或 危險形態有變才扣分
                    punish_countdown = punish_countdown - 1
                    danger_flag = 1
                    danger_flag_time = pygame.time.get_ticks()#---------------------取得偵測到危險駕駛的時間

            if  (preds_long + 1 == 8)or(preds_long + 1 == 11)or(preds_long + 1 == 12):
                if (danger_flag_long == 0) or(not(prev_preds_long == preds_long)):#3秒後 或 危險形態有變才扣分
                    punish_countdown = punish_countdown - 1
                    danger_flag_long = 1
                    danger_flag_long_time = pygame.time.get_ticks()#---------------------取得偵測到危險駕駛的時間
            if (horn_num > max_horn_num):
                punish_countdown = punish_countdown - 1
    ...
    while ser_rfid.in_waiting:#----------------get rifd input
    ...
    if current_time - danger_flag_time > 1500:#---------------------------偵測到危險駕駛緩衝時間三秒
        danger_flag = 0
    if current_time - danger_flag_long_time > 3000:
        danger_flag_long = 0
...
#////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    points_text =  my_font.render(str(punish_countdown), True, red)
    ...
    point_state_text = my_font.render('points left:', True, red)
    ...
    winScreen.blit(rfid_text_surface, (500, 150))
    winScreen.blit(points_text,(500, 170))
    ...
    winScreen.blit(rfid_state_text, (10, 150))
    winScreen.blit(point_state_text, (10, 170))
